...
| RATE_LIMIT                              | int    | 250                                                              | Maximum number of requests allowed within the rate limit time window                                                          |
| RATE_LIMIT_PERIOD                       | string | 1-M                                                              | Rate limit window in the format [number]-[unit] where unit is one of `S` (seconds), `M` (minutes), `H` (hours), or `D` (days) |
